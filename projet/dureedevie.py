...
class App:
    ...
    def __update_actors(self) -> None:
        ...
        for lapin_sprite in self.lapins.sprites():
            lapin = lapin_sprite._actor
            if lapin.energie <= 0 or not lapin.vieillir():
                lapin_sprite.kill()
            # Retirer les renards morts
        for renard_sprite in self.renards.sprites():
            renard = renard_sprite._actor
            if renard.energie <= 0 or not renard.vieillir():
                renard_sprite.kill()

         # Lapins mangent les plantes
        collisions = pygame.sprite.groupcollide(self.lapins, self.plants, False, True)
        for lapin, plantes_mangees in collisions.items():
            for plante in plantes_mangees:
                lapin._actor.change_energie(plante.energie_fournie)

        # Renards mangent les lapins
        collisions = pygame.sprite.groupcollide(self.renards, self.lapins, False, True)
        for renard, lapins_manges in collisions.items():
